...
export const updateOrderStatus = createAsyncThunk('admin/updateOrderStatus', async ({ id, status }, { rejectWithValue }) => {
    try {
        ...
        const { data } = await axios.put(`/api/v1/order/admin/order-status/${id}`, { status }, config)
...
export const deleteReview = createAsyncThunk('admin/deleteReview', async ({ productId, reviewId }, { rejectWithValue }) => {
    try {
        ...
        const { data } = await axios.delete(
            `/api/v1/products/admin/review-delete?productId=${productId}&id=${reviewId}`
        );
    ...
    extraReducers: (builder) => {
        ...
        builder
            ...
            .addCase(deleteReview.rejected, (state, action) => {
                state.loading = false;
                state.success = false;
                state.error = action.payload || 'Failed to Product Review.';
            });
